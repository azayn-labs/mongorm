# MongORM Primitives

In this section, you will find all the available field types that you can use to define your model schema. Each field type has its own set of methods that you can use to perform queries on the model. You can find the documentation for each field type by clicking on the links below.

## Available Field Types

- [ObjectIDField](../mongorm/primitives/oid.go)
- [StringField](../mongorm/primitives/string.go)
- [Int64Field](../mongorm/primitives/int.go)
- [Float64Field](../mongorm/primitives/float.go)
- [BooleanField](../mongorm/primitives/bool.go)
- [TimestampField](../mongorm/primitives/timestamp.go)
- [GenericField](../mongorm/primitives/generic.go)
